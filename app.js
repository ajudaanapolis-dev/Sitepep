// Tema azul/verde médico aplicado via Tailwind config + sidebar compartilhada
function renderShell(active, titulo) {
  ...
  const navHtml = itens.map(i => `
    <a href="${i.href}" class="flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium ${active === i.href ? 'bg-teal-700 text-white' : 'text-slate-700 hover:bg-teal-50'}">
  ...
  <div class="min-h-screen bg-slate-50 md:flex">
    <header class="flex h-14 items-center justify-between border-b bg-white px-4 md:hidden">
      <div class="font-semibold text-teal-800">Clínica PEP</div>
    ...
    <aside class="hidden min-h-screen w-72 flex-col border-r bg-white md:flex">
      <div class="border-b bg-gradient-to-br from-teal-700 to-sky-700 px-6 py-5">
        <div class="text-xl font-bold text-white">Clínica PEP</div>
        <div class="text-sm text-teal-50">Prontuário Eletrônico</div>
      </div>
